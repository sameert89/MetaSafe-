...
GenericGFPoly.prototype = {
  init: function (field, coefficients) {
    ...
    if (coefficientsLength > 1 && coefficients[0] === 0) {
      ...
      if (firstNonZero == coefficientsLength) {
        ...
        this.coefficients = coefficients.subarray(
          firstNonZero,
          coefficientsLength
        );
      }
  ...
  addOrSubtract: function (other, buf) {
    if (this.field !== other.field) {
      throw new Error(
        'IllegalArgumentException("GenericGFPolys do not have same GenericGF field")'
      );
    }
    if (this.isZero()) {
      return other;
    ...
    var sumDiff = buf
      ? buf.subarray(0, largerCoefficients.length)
      : new Int32Array(largerCoefficients.length);
      ...
      for (var d = this.cachedGenerators.length; d <= degree; d++) {
        var nextGenerator = lastGenerator.multiply(
          new GenericGFPoly(
            ...
            new Int32Array([
              1,
              this.field.exp(d - 1 + this.field.generatorBase),
            ])
          )
        );
